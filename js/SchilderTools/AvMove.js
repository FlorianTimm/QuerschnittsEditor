...
class AvMove {
    ...
    addAufstellButton() {
        ...
        if (!("Otaufstvor" in this.abschnitt.inER)) {
            PublicWFS.addInER(this.abschnitt, "Otaufstvor", this.daten.ereignisraum_nr, AvMove._addInER_Callback, undefined, this);
        } else {
            AvMove._wfsAddAufstell(this)
        }
    }

    static _addInER_Callback(xml, _this) {
        //console.log(_this.daten)
        Aufstellvorrichtung.loadAbschnittER(_this.daten, _this.abschnitt, AvMove._wfsAddAufstell, _this)
    }
