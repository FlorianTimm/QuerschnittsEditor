...
export default class SAPDelete extends Tool {
    ...
    _featureSelected(event: SelectEvent) {
        ...
        let auswahl = <StrassenAusPunkt>event.selected[0];
    ...
    _featureDelete() {
        let feature = this._select.getFeatures().getArray()[0] as StrassenAusPunkt;
        console.log(feature);
        let update = '<wfs:Delete typeName="Otstrauspkt">\n' +
            '	<ogc:Filter>\n' +
            '		<ogc:And>\n' +
